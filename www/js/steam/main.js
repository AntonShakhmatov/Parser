...
{
	var win = window.open( 'localhost:8000/parser/gotflash','gotflash','width=536,height=546,resize=yes,scrollbars=yes');
	win.focus();
}
...
function PageTab( tab, delta, max, params )
{
	...
	if ( tabStart[tab] > tabMax[tab] )
	{
		...
		new Ajax.Updater(
				'tab_' + tab + '_items',
				'localhost:8000/parser/search/tab',
...
function ScrollSmallCaps( name, delta, pageSize, totalCount, params )
{
	...
	if ( elem.curPos > elem.maxLoaded )
	{
		...
		new Ajax.Updater(
				targetid,
				'localhost:8000/parser/search/smallcapscroll',
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			window.setTimeout( function() {
				if ( oElemState.bWantsHover && !oElemState.bAjaxRequestMade ) {
					...
					$J.get( 'localhost:8000/parser/' + strUrlTarget, rgAjaxParams ).done( function( html )
					{
						var $Content = $J(html);
						$Content.hide();
...
function AddToWishlist( appid, divToHide, divToShowSuccess, divToShowError, navref, divToHide2 )
{
	if ( !g_AccountID )
	{
		...
		.done( function() {
			window.location.href = 'localhost:8000/parser//login?redir=app/' + appid;
		} );
		return;
	}

	var url = 'localhost:8000/parser/api/addtowishlist';
	if ( navref )
	{
...
{
	var url = 'localhost:8000/parser/api/removefromwishlist';
	if ( navref )
	{
		MakeNavCookie( navref, url );
	}
...
{
	$J.post( 'localhost:8000/parser/recommended/ignorerecommendation/', {
...
function RecommendGame( appid, steamworksappid, comment, rated_up, is_public, language, received_compensation, divBtn, onSuccessFunc, divToShowError, navref, bDisableComments )
{
	...
	var url = 'localhost:8000/parser/friends/recommendgame';
	if ( navref )
		MakeNavCookie( navref, url );
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'localhost:8000/parser/recommended/ignorerecommendation/', {
		method: 'post',
...
{
	window.location = 'localhost:8000/parser/search/?term=' + $Term;
}
...
function SearchTimeout( $Term, value, $SuggestionsCtn, $Suggestions )
{
	if ( value )
	{
		...
		$J.get( 'localhost:8000/parser/search/suggest', parameters).done( function( html ) {
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		var url = 'localhost:8000/parser/tagdata/recommendedtags';
		...
		$J.get( url, {ll: 'english'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'localhost:8000/parser/tag/en/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	{
		posts.push( $J.post( 'localhost:8000/parser/checkout/addfreelicense/' + subids[i], { ajax: true, sessionid: g_sessionID }) );
	}
...
function AddFreeBundle( bundleid, strDisplayName )
{
	...
	var posts = [];
	posts.push( $J.post( 'localhost:8000/parser/checkout/addfreebundle/' + bundleid, { ajax: true, sessionid: g_sessionID }) );
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'localhost:8000/parser/account/setlanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
	.done( function() {
		if ( bStayOnPage )
			Modal.Dismiss();
		else
		{
				...
				window.location = 'localhost:8000/parser/account/languagepreferences/';
...
function Logout()
{
	var $Form = $J('<form/>', {'action': 'localhost:8000/parser/logout/', 'method': 'POST' } );
...
function addToCart( subid, dedupe )
{
	try
	{
		...
		if ( !$Form.length )
		{
			$Form= $J('<form/>', { name: 'add_to_cart_' + subid, action: 'localhost:8000/parser/cart/', method: 'POST', style: 'display: none;' } );
...
function addBundleToCart( bundleid, dedupe )
{
	try
	{
		...
		if ( !$Form.length )
		{
			$Form= $J('<form/>', { name: 'add_bundle_to_cart_' + bundleid, action: 'localhost:8000/parser/cart/', method: 'POST', style: 'display: none;' } );
			$Form.append( $J('<input/>', { type: 'hidden', name: 'action', value: 'add_to_cart' } ) );
...
function RecordAJAXPageView( url )
{
	...
	{
		var rgURLs = [ 'localhost:8000/parser/', 'localhost:8000/parser/' ];
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	$J.post(
		'localhost:8000/parser/curators/ajaxfollow',
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	$J.post(
		'localhost:8000/parser/curators/ajaxignore',
